// client/src/components/approvals/HeadConsultantApproval.js
...
const HeadConsultantApproval = ({ report, projectId, inspectionId, onApprovalChange }) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await axios.post(
        `/api/approvals/reports/${report.id}/approve/head_consultant`,
        { comment },
        {
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await axios.post(
        `/api/approvals/reports/${report.id}/reject/head_consultant`,
        { comment },
        {
          headers: { Authorization: `Bearer ${token}` }
        }
            ...
            <Box>
              ...
              <VStack align="stretch" spacing={2}>
                ...
                <Text>
                  ...
                  <Badge colorScheme="orange">{report.status?.replace(/_/g, ' ')}</Badge>
                </Text>
              </VStack>
            </Box>
